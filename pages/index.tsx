    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50 text-gray-900">
      ...
      <section className="relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16">
          ...
          <div className="relative mt-12 mx-auto max-w-5xl bg-white/60 backdrop-blur-xl border border-white/20 rounded-3xl p-4 sm:p-6 shadow-2xl">
      ...
      <main className="relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20 pb-20">
              ...
              <Carousel>
                {featuredContent.featuredProposals.map((proposal) => (
                  <div key={proposal._id} className="h-full">
                    <div className="bg-white/60 backdrop-blur-xl border border-white/20 rounded-3xl p-6 shadow-xl h-full">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                          ...
                          <span className="inline-flex items-center text-xs px-2 py-0.5 rounded-full bg-white/60 border border-white/20"><HiStar className="h-4 w-4 mr-1" /> Featured</span>
                        </div>
                      </div>
              ...
              <Carousel>
                {featuredContent.featuredBusinesses.map((biz) => (
                  <div key={biz._id} className="h-full">
                    <div className="bg-white/60 backdrop-blur-xl border border-white/20 rounded-3xl p-6 shadow-xl h-full text-center">
                      ...
                      <Link href={`/users/${biz._id}`}>
                        <a className="inline-block px-4 py-2 rounded-lg bg-white/60 border border-white/20 hover:bg-white/20 transition">View Profile</a>
                      </Link>
                    </div>
                  </div>
                ))}
              ...
              <Carousel>
                {featuredContent.featuredEvents.map((evt) => (
                  <div key={evt._id} className="h-full">
                    <div className="bg-white/60 backdrop-blur-xl border border-white/20 rounded-3xl overflow-hidden shadow-xl h-full">
                      {evt.image?.url && (
                        // eslint-disable-next-line @next/next/no-img-element
                      ...
                      <div className="p-6">
                        ...
                        <Link href={`/events/${evt._id}`}>
                          <a className="mt-4 inline-block px-4 py-2 rounded-lg bg-white/60 border border-white/20 hover:bg-white/20 transition">View Details</a>
                        </Link>
                      </div>
                    </div>
                  </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {featuredContent.featuredProposals.slice(0, 6).map((p) => (
                  <div key={p._id} className="bg-white/60 backdrop-blur-xl border border-white/20 rounded-3xl p-6 shadow-xl hover:shadow-2xl transition">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {["Post Proposal", "Connect", "Match", "Build Together"].map((step, idx) => (
                <div key={step} className="bg-white/60 backdrop-blur-xl border border-white/20 rounded-3xl p-6 text-center shadow-xl hover:-translate-y-0.5 transition transform">
          ...
          <section>
            ...
            <div className="space-y-3">
              {[{
                ...
                <details key={i} className="group bg-white/60 backdrop-blur-xl border border-white/20 rounded-2xl p-4">
            ...
            <div className="flex flex-col md:flex-row items-center gap-4">
              ...
              <form className="flex w-full md:w-auto gap-3" onSubmit={(e) => { e.preventDefault(); alert("Subscribed"); }}>
                <input type="email" required placeholder="Your email"
                       className="flex-1 md:w-96 px-4 py-3 rounded-xl bg-white/60 border border-white/20 placeholder-zinc-400 focus:outline-none" />
          ...
          {/* CTA Banner */}
          <section>
            <div className="relative overflow-hidden rounded-3xl p-8 md:p-12 border border-white/20 bg-white/60 backdrop-blur-xl text-center">
